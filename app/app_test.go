...
func TestWasmdExport(t *testing.T) {
	db := db.NewMemDB()
	gapp := NewFuryunderverseApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyAppOptions{}, emptyWasmOpts)
	...
	newGapp := NewFuryunderverseApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyAppOptions{}, emptyWasmOpts)
...
func setGenesis(gapp *FuryunderverseApp) error {
	genesisState := NewDefaultGenesisState(gapp.appCodec)
	stateBytes, err := json.MarshalIndent(genesisState, "", " ")
	if err != nil {
		return err
