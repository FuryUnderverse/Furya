...
echo $PASS | furyad tx provider set-datasource eth_price $(cat ../address.txt) "test eth price" --from $USER --chain-id $CHAIN_ID -y
...
echo $PASS | furyad tx provider set-testcase eth_price_testcase $(cat ../address.txt) "test eth price testcase" --from $USER --chain-id $CHAIN_ID -y
...
echo $PASS | furyad tx provider set-oscript oscript_eth $(cat ../address.txt) "test eth price oracle script" --ds eth_price --tc eth_price_testcase --from $USER --chain-id $CHAIN_ID -y